import db/schema.{type ThirdB074}
...
pub fn render(thirdb074list: List(ThirdB074)) {
  // Create sketch cache
  let assert Ok(cache) =
    sketch_options.node()
    |> sketch_lustre.setup()
  ...
  let table_items =
    ...
    |> list.map(fn(b074) {
      html.tr([], [
        html.th([attribute("scope", "row")], [text(b074.id |> int.to_string)]),
        html.td([], [text(b074.from_sta_code)]),
        html.td([], [text(b074.sta_code)]),
        html.td([], [text(b074.sta_name)]),
      ])
    })
